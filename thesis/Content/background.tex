...
As we are aiming for a pixel-/voxelwise segmentation of our input, we have to capture the object boundaries and regarding this task, we could improve the quality of the output by using color-based features of SLIC superpixels (\cite{achanta2010slic}) instead of the values of pre-processed image patches to describe image regions. 
...
This corresponds to the steps that are iteratively done in previous proposed semi-supervised setups for boosting methods like ASSEMBLE \cite{bennett2002exploiting} or SemiBoost \cite{mallapragada2009semiboost} where they iteratively assign labels to the unlabeled data using the original labeled data and labeled from previous iteration, if available. 
...
\todoRef{add SLIC reference somewhere!}
