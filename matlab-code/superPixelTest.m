...
if (size(test_img,3) == 3)
    lab_img = rgb2gray(test_img);
end

superpixel_size = round(min([size(test_img,1), size(test_img,2)]) / 11.5);
regularizer = 0.1;

super_lab = double(getSuperPixels(single(lab_img), superpixel_size, regularizer));
grad_lab = repmat(imgradient(super_lab),1,1,size(test_img,3));
